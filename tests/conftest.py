...
def spark() -> Iterator[SparkSession]:
    ...
    Generally, "session" will be chosen as the fixture only needs to be
    set up once and this will make the tests run faster
    """
    spark = (
        SparkSession.builder.appName("dlh_utils unit tests")
        .master("local")
        .getOrCreate()
    )
    yield spark
