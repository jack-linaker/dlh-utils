...
regex_data = [
    ["NHS No", r"^\d{10}$", "de_utils", ""],
    ["NIN", r"^\s*[a-zA-Z]{2}(?:\s*\d\s*){6}[a-dA-D]{1}?\s*$", "Stack Overflow", ""],
    ["UK phone no", r'"^(?:0|\+?44)(?:\d\s?){9,10}$"', "RegExLib.com", ""],
    ["UK Child Benefit", r"^[A-Z]{3}\d{8}$", "RegExLib.com", ""],
    [
        "UK Passport",
        r'"[0-9]{10}[a-z]{3}[0-9]{7}[U,M,F]{1}[0-9]{9}"',
        "RegExLib.com",
        "",
    ],
    ["UK VAT", r"^\d{9}$", "de-utils", ""],
    [
        "UK Vehicle Reg",
    ...
    [
        "filepath_1",
        r"/^(?:[\w]\:|\/)(\/[a-z_\-\s0-9\.]+)+\.(txt|gif|pdf|doc|docx|xls|xlsx|js)$",
    ...
    [
        ...
        r"[a-zA-Z0-9\s]",
        "",
        "Check if it's a free text field. Any number of words having any number of "
        "spaces",
    ],
]
